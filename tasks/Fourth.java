public class Fourth {
    public static String textChanger(int numWord, int symbolsInLine, String text)//1 - split text into lines with a fixed number of characters
    {
        ...
        boolean error = true;
        int count = 0;
        while (text.indexOf(" ")==0)//remove extra spaces
        ...
        if (esseText.length == numWord)//checks correct number of words
            for (String cWord:esseText)
                if (cWord.length()>symbolsInLine)//checks length of words
                {
                    error = false;
                    break;
                }
                else
                if (count+cWord.length()<=symbolsInLine)//checks if current length of line is available
                {
                    count+=cWord.length();
    ...
    public static int countUniqueBooks(String bookText, char endOfBooks)//10-function returns the number of unique letters between "endOfBooks"
    {
        ...
        {
            String tempLine = bookText.substring(bookText.indexOf(endOfBooks)+1,bookText.indexOf(endOfBooks,bookText.indexOf(endOfBooks)+1));//take one part
            bookText=bookText.substring(bookText.indexOf(endOfBooks,bookText.indexOf(endOfBooks)+1)+1);//removing this part from begin string
