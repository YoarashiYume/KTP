...
public class Main {
	public static double computeArea(Point3d first, Point3d second,	Point3d third) {
		if (first.equals(second) || first.equals(third) || second.equals(third)) return 0.0;
		double a = Math.sqrt(Math.pow(first.getX() - second.getX(), 2) + Math.pow(first.getY() - second.getY(), 2));
		double b = Math.sqrt(Math.pow(first.getX() - third.getX(), 2) + Math.pow(first.getY() - third.getY(), 2));
		double c = Math.sqrt(Math.pow(second.getX() - third.getX(), 2) + Math.pow(second.getY() - third.getY(), 2));
		double p = (a + c + b) / 2;
